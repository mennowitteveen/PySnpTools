...
#import sys
#import doctest
#from pysnptools.util.mapreduce1.runner import Local, LocalMultiProc, LocalInParts, _LocalFromRanges
...
#        assert "A" == _LocalFromRanges([2,4]).run(A(10,0))
#        assert "A" == _LocalFromRanges([2,9,10]).run(A(3,0))
#        assert "A" == _LocalFromRanges([2,9,10]).run(A(3,10))
...
#            assert "A" == _LocalFromRanges(list).run(A(a_work_count,b_work_count))
